...
class BuyerChangePasswordScreen extends StatefulWidget {
  ...
  @override
  State<BuyerChangePasswordScreen> createState() => _BuyerChangePasswordScreenState();
}

class _BuyerChangePasswordScreenState extends State<BuyerChangePasswordScreen> {
  final _formKey = GlobalKey<FormState>();
  final BuyerPasswordResetRepo _apiService = BuyerPasswordResetRepo();
  ...
  Future<void> _handleChangePassword() async {
    ...
    if (token == null || token.isEmpty) {
      CustomSnackbar.showError(context, 'Authentication required. Please login again.');
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        CustomSnackbar.showError(
          context,
          'Failed to change password: ${e.toString().replaceFirst('Exception: ', '')}',
        );
      }
    } finally {
      if (mounted) {
